...
var svg = d3.select("svg"); // setting svg size
...
var innerHeight = height - margin.top - margin.bottom; // renders the data into the svg

var render = function render(data) {
  var title = "Average Viewers vs. Followers";

  var xValue = function xValue(d) {
    return d.averageViewers;
  };

  var xAxisLabel = "Average Viewers (Thousand)";
  ...
  var circleRadius = 10;
  var yAxisLabel = "Followers (Thousand)";
  var xScale = d3.scaleLog().base(2).domain(d3.extent(data, xValue)).range([0, innerWidth]).nice();
  var yScale = d3.scaleLog().base(2).domain(d3.extent(data, yValue)).range([innerHeight, 0]).nice();
  var g = svg.append("g").attr("transform", "translate(".concat(margin.left, ",").concat(margin.top, ")"));
  var xAxis = d3.axisBottom(xScale).tickSize(-innerHeight).tickPadding(10).tickFormat(function (d) {
    return Math.round(d / 1000);
  });
  var yAxis = d3.axisLeft(yScale).tickSize(-innerWidth).tickPadding(10).tickFormat(function (d) {
    return Math.round(d / 1000);
  });
  var yAxisG = g.append("g").call(yAxis);
  yAxisG.selectAll(".domain").remove();
  yAxisG.append("text").attr("class", "axis-label").attr("y", -90).attr("x", -innerHeight / 2).attr("fill", "black").attr("transform", "rotate(-90)").attr("text-anchor", "middle").text(yAxisLabel).style("font-size", 20);
  var xAxisG = g.append("g").call(xAxis).attr("transform", "translate(0,".concat(innerHeight, ")"));
  xAxisG.select(".domain").remove();
  xAxisG.append("text").attr("class", "axis-label").attr("y", 64).attr("x", innerWidth / 2).attr("fill", "black").text(xAxisLabel).style("font-size", 20);
  g.selectAll("circle").data(data).enter().append("circle").attr("cy", function (d) {
    return yScale(yValue(d));
  }).attr("cx", function (d) {
    return xScale(xValue(d));
  }).attr("r", circleRadius).attr("fill", "#8C44F7").on("mouseover", mouseover).on("mousemove", mousemove).on("mouseleave", mouseleave);
  g.append("text").attr("class", "title").attr("y", -45).attr("x", 90).text(title).style("font-size", 32);
  var lineGen = d3.line().x(function (d) {
    return xScale(xValue(d));
  }).y(function (d) {
    return yScale(yValue(d));
  }).curve(d3.curveBasis); // Add the valueline path.
...
var tooltip = d3.select("#app").append("div").style("opacity", 0).attr("class", "tooltip").style("position", "absolute").style("background-color", "white").style("border", "solid").style("border-width", "1px").style("border-radius", "5px").style("padding", "10px"); // mouseover fn sets the html in the tooltip with the correct data

var mouseover = function mouseover(event, d) {
  tooltip.html("Channel: <strong>".concat(d.channel, "</strong>\n      <br>Followers: <strong>").concat(d.followers, "</strong>\n      <br>Averag Viewers: <strong>").concat(d.averageViewers, "</strong>")).transition().duration(150).style("opacity", 1);
}; // tooltip will follow the mouse movement


var mousemove = function mousemove(event, d) {
  tooltip.style("transform", "translateY(-55%)").style("left", "".concat(event.x + 10, "px")).style("top", "".concat(event.y - 30, "px"));
...
},{"d3":"node_modules/d3/index.js","./styles.css":"styles.css"}],"node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/Information-visualization-course.e31bb0bc.js.map